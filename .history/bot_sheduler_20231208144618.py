...
def get_from_minio_task():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT image_reference_link FROM history_image_links WHERE user_id = {task[1]}")
        res =(cursor.fetchall()[0][0].split(';;'))
        result = [''.join(group) for group in ''.join(res).split(' ') if group != '']
        new_array = ' '.join(result).replace('jpg', ' jpg').replace('png', ' png').split()
        cursor.close()
        conn.close() 
        print(new_array)
        while len(images_state.minio_image_list) != task[3]:
                ...
                for n in range(0, len(images_path), chunk_size):
                    ...
                    for image_ref_path in images_path[n : n + chunk_size]:
                        print(image_ref_path.replace('jpg', ''))
                        if image_ref_path.replace('jpg', '') not in new_array:
