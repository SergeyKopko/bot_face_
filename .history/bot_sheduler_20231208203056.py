...
async def get_task_from_db_queue() -> Union[tuple, None]:
    """Функция получает задачу из бд таблицы queue."""
    try:
    
        with await loop.run_in_executor(None, psycopg2.connect,
            user=env["POSTGRES_USER"],
            password=env["POSTGRES_PASSWORD"],
            database=env["DB_NAME"],
            host=env["DB_HOST"],
        ) as db:
            first_request_in_queue = await loop.run_in_executor(None, get_first_request_type_in_queue, db)
            return first_request_in_queue
    except Exception:
        logging.exception("Возникла ошибка в send_request_to_db_queue")


async def get_from_minio_task():
    """Подзадача для получения задачи из БД и получения фото."""
    if not queue_state.get_from_minio_task.empty():
        return

    task = await get_task_from_db_queue()
    if not task:
        return
    ...
    try:
        ...
        async def get_images(images_state, images_path):
            tasks = []
            chunk_size = 100
        ...
        while len(images_state.minio_image_list) != task[3]:
            images_state.minio_image_list = []
            await get_images(images_state, images_path)

        # извлекаем фото пользователя
        object_data = await loop.run_in_executor(None, MINIO_CLIENT.get_object, BUCKET_NAME, task[11])
...
async def start_swapping_task():
    ...
    logging.info(f"start start_swapping_task {task[0]} {task[2]} {task[3]}")
    ...
    logging.info(f"finish start_swapping_task {task[0]} {task[2]} {task[3]}")
    queue_state.swap_task.put(images_state)


async def async_execute(images_state: ImagesState, minio_image_list, watermark, minio_user_image):
    try:
        await loop.run_in_executor(
            None,
            face_swapping_process,
            input_type="Image",
            image_path=minio_image_list,
            source_path=minio_user_image,
...
async def put_to_minio_task():
    """Функция выполняет подзадачу для выгрузки фото через minio client. Также вносит необходимые изменения в бд."""
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            ...
            for i, image_target_link in enumerate(images_state.image_target_byte)
        ]
    ...
    try:
        ...
        with await loop.run_in_executor(None, psycopg2.connect,
            ...
            if task[2] in [IMAGE_FREE, IMAGE_FOR_REFERAL]:
                collage_link = await loop.run_in_executor(
                    None,
                    make_ref_collage,
                    images_state.image_target_byte,
                    task[1],
                ...
                await loop.run_in_executor(None, update_image_colage_link, task[4], collage_link, db)
            if task[2] in [IMAGE_COLLAGE_50, IMAGE_COLLAGE_100]:
                ...
                await loop.run_in_executor(None, update_image_colage_link, task[4], collage_link, db)
            await loop.run_in_executor(None, remove_requests_from_queue, task[0], task[4], db)
    except IndexError:
        if task[3] != task[7]:
            if task[3] % 10 == 0:
                target_image_links = await loop.run_in_executor(None, remove_requests_from_queue, task[0], task[4], db)
            return
        target_image_links = await loop.run_in_executor(None, remove_requests_from_queue, task[0], task[4], db)
    except Exception:
        logging.exception(
...
# Функция реализует логику для выполнения задач из таблицы check_user_image_queue.
scheduler.add_job(
...
# Функция выполняет подзадачу для получения задачи из БД из таблицы queue и загрузки изображений через minio client
scheduler.add_job(
    get_from_minio_task,
    trigger="interval",
...
# Функция выполняет подзадачу для получения задачи из БД из таблицы queue Также вносит необходимые изменения в бд.
scheduler.add_job(
    start_swapping_task,
    trigger="interval",
    seconds=GET_FROM_MINIO_TASK_INTERVAL,
