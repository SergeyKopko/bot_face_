...
async def get_task_from_db_queue() -> Union[tuple, None]:
    ...
    try:
        async with asyncpg.connect(
            ...
            first_request_in_queue = await get_first_request_type_in_queue(db)
...
async def get_from_minio_task():
    ...
    try:
        ...
        async def get_image(image_ref_path):
            await get_object_from_minio(image_ref_path, images_state)

        tasks = [get_image(image_ref_path) for image_ref_path in images_path]
...
async def put_to_minio_task():
    ...
    try:
        ...
        async with asyncpg.connect(
            ...
            await add_to_history_image_links(
                task[4],
                task[1],
                task[7],
            ...
            if task[2] in [IMAGE_FOR_REFERAL]:
                await add_history_collage_links(
                    task[1],
                    task[2],
                    task[4],
                    db,
            ...
            if task[2] in [IMAGE_FREE, IMAGE_FOR_REFERAL]:
                collage_link = await make_ref_collage(
                    images_state.image_target_byte,
                    task[1],
                    task[4],
                    1187,
                ...
                await update_image_colage_link(task[4], collage_link, db)
            if task[2] in [IMAGE_COLLAGE_50, IMAGE_COLLAGE_100]:
                collage_link = await make_collage(
                    images_state.image_target_byte,
                    task[1],
                    task[4],
                    1187,
                ...
                await update_image_colage_link(task[4], collage_link, db)
            await remove_requests_from_queue(task[0], task[4], db)
    except IndexError:
        if task[3] != task[7]:
            if task[3] % 10 == 0:
                async with asyncpg.connect(
                    ...
                    target_image_links = await remove_requests_from_queue(
                        task[0],
                        task[4],
                        db,
                    )
