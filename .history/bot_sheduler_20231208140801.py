...
def get_from_minio_task():
    ...
    try:
        ...
        with psycopg2.connect(
            ...
            cursor = db.cursor()
            cursor.execute(f"SELECT image_reference_link FROM history_image_links ")
            res = [i for i in cursor.execute(f"SELECT image_reference_link FROM history_image_links ")]
            print(res)
